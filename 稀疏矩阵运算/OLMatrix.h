...
void CreateOLMatrix(Crosslist* M)
{
	for (int k = 0; k < M->tu; k++)
	{
		...
		//以下两段是重点，对每个加入的结点都要在两个方向上拉链
		if (M->rhead[icur] == NULL || M->rhead[icur]->j > jcur)
		{
			//如果结点放在行的第一个位置(还没有元素/列最小)
			p->right = M->rhead[icur];
			M->rhead[icur] = p;
		}
		else
		{
			OLNode* pcur = M->rhead[icur];
			for (; (pcur->right) && (pcur->right->j < jcur); pcur = pcur->right);
			//通过一个循环定位(如果右边还有且列号更小，就不断向右)
			p->right = pcur->right;
			pcur->right = p;
		}

		if (M->chead[jcur] == NULL || M->chead[jcur]->i > icur)
		{
			p->down = M->chead[jcur];
			M->chead[jcur] = p;
		}
		...
		{
			OLNode* pcur = M->chead[jcur];
			for (; (pcur->down) && (pcur->down->i < icur); pcur = pcur->down);
			p->down = pcur->down;
			pcur->down = p;
		}
	}
